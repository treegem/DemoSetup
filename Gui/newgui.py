...
class MainGui(QWidget):
    ...
    def closeEvent(self, event):
        quit_msg = "Are you sure you want to quit? All data not saved will be lost!"
...
class LabJackGui(QWidget):
    ...
    def __init__(self, backend):
        ...
        viewTime = lambda: self.label_time.setText(str(self.backend.getPulseTime()))
        ...
        self.box_pulsetime.editingFinished.connect(self.enterTime)
    ...
    def updateModLabJack(self):
        ...
        if modLabJack == 0:
            ...
            self.box_pulsetime.setEnabled(False)
            #Print a status
            ...
            self.box_pulsetime.setEnabled(True)
    ...
    #Enter a pulsetime (temporary)
    def enterTime(self):
        pulseTime = self.box_pulsetime.value()
        self.label_stat2.setText("Entering pulsetime...")
        if self.backend.timeManual(pulseTime):
            self.label_stat2.setText("Pulsetime set")
        else:
            self.label_stat2.setText("Could not set pulsetime")
    ...
    def plotTrigger(self):
        ...
        while self.plot:
            ...
            if self.isVisible():
                ...
                self.mpl_diff.figure.colorbar(imageDiff, ax=self.axesDiff, shrink=0.9)
                #And send the plot signal
                self.sigPlot.emit()
                time.sleep(1)
...
class OdmrGui(QWidget):
    ...
    def updateMod(self):
        ...
        if modLabJack == 0 and modCam == 0:
            ...
            self.box_pulse.setEnabled(False)
            #Print a status
            ...
            self.box_pulse.setEnabled(True)
    ...
    def measure(self):
        ...
        pulse = self.box_pulse.value()
        self.label_stat2.setText("Starting ODMR...")
        if self.backend.startOdmr(minfreq, maxfreq, stepfreq, count, pulse):
...
class RabiGui(QWidget):
    ...
    def measure(self):
        ...
        if self.backend.startRabi(mintime, maxtime, steptime, count, freq):
