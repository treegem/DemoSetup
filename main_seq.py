...
class Main():
    ...
    def __init__(self):
        ...
        self.seq = 'Off'  # Current sequence active
        ...
        #Odmr information
        self.odmr = [[], [], [], []]  # Array for storing measured odmr data. Voltage, Frequency, Intensity, average intensity
    ...
    def readImage(self):
        if self.modCam >= 1:
            ...
            bgAoi = self.imageBackground[(self.aoiSoft[1]):(self.aoiSoft[1] + self.aoiSoft[3]), (self.aoiSoft[0]):(self.aoiSoft[0] + self.aoiSoft[2])]
    ...
    #For setting the sequence while in manual mode
    ...
    def writeFpgaSettings(self, chMw, chLaser):
        if self.modFpga == 1 and self.modLabJack <= 1:
            try:
                ...
                self.fpga.channel_map.update({'Mw': chMw, 'Laser': chLaser, ('ch%i' % chMwOld): chMwOld, ('ch%i' % chLaserOld): chLaserOld})
                return True
            except:
                return False
        else:
    ...
    #Output a pulses of length time once in every reptime-Period. All times in ns
